...
async function main() {
  ...
  if (argv['sponsor-wallet-address']) console.log(`${colors.fg.cyan}● Sponsor Wallet Address:${colors.reset} ${argv['sponsor-wallet-address']}`);

  // Define the .permaweb directory in the home directory
  const permawebDir = join(homedir(), '.permaweb');
  if (!existsSync(permawebDir)) {
    console.log(`${colors.fg.blue}Creating directory:${colors.reset} ${permawebDir}`);
    mkdirSync(permawebDir);
  }

  // Define the project-specific directory inside .permaweb
  const projectDir = join(permawebDir, projectName);
  ...
  const walletPath = join(projectDir, 'wallet.json');
  ...
  const configPath = join(process.cwd(), '.perma-deploy', 'config.json');
  if (existsSync(configPath)) {
    try {
      const existingConfig = JSON.parse(readFileSync(configPath, 'utf8'));
      if (existingConfig.sigType === 'arweave' && existingConfig.walletPath && existsSync(existingConfig.walletPath)) {
  ...
  if (sigType === 'arweave') {
    ...
    console.log(`${colors.fg.green}Make sure to fund this wallet with AR or Turbo credits before deployment if the file size is greater than 100kb and no sponsor wallet is used.${colors.reset}`);
  ...
  const permaDeployDir = join(process.cwd(), '.perma-deploy');
  if (!existsSync(permaDeployDir)) {
    console.log(`${colors.fg.blue}Creating .perma-deploy directory...${colors.reset}`);
    mkdirSync(permaDeployDir);
  }
  ...
  let config = {
    ...
    sigType,
    walletAddress,
    sponsorWalletAddress: argv['sponsor-wallet-address'] || null,
  };
  ...
  if (existsSync(configPath)) {
    ...
    try {
      ...
      config = {
        ...
        projectName: argv['project-name'] !== basename(process.cwd()) ? projectName : existingConfig.projectName,
        ...
        sponsorWalletAddress: argv['sponsor-wallet-address'] !== undefined ? argv['sponsor-wallet-address'] : existingConfig.sponsorWalletAddress,
      };
    } catch (error) {
  ...
  try {
    ...
    if (existsSync(packageJsonPath)) {
      ...
      if (config.sponsorWalletAddress) {
        deployCommand += ` --sponsor-wallet-address ${config.sponsorWalletAddress}`;
      }
...
${config.sponsorWalletAddress ? `- Sponsor Wallet Address: ${config.sponsorWalletAddress}` : ''}

### Deployment Instructions

1. Make sure your environment is properly set up:
   ${config.sigType === 'arweave' ? 
     `- The wallet at ${config.walletPath} should be funded with AR or Turbo credits if no sponsor wallet is used.` : 
     `- Set the DEPLOY_KEY environment variable with your ${config.sigType} private key.`}
   ${config.sponsorWalletAddress ? `- Ensure your wallet address is whitelisted by the sponsor at ${config.sponsorWalletAddress} for shared credits.` : ''}
...
`;
  ...
  if (!hasAntConfig && !hasArnsName && !hasUndername) {
    console.log(`${colors.bg.green}${colors.fg.black} npx nitya deploy and top up your wallet with turbo credits if not using a sponsor wallet ${colors.reset}`);
  } else {
    console.log(`${colors.bg.green}${colors.fg.black} Granting controller access and top up your wallet with turbo credits if not using a sponsor wallet ${colors.reset}`);
  }
}
